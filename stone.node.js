...
module.exports =  {
    ...
    isArray: function(t){
        return this.getType(t) == 'array';
    },
    extend: function() {
        function _classOf(o,note){
        ...
        function _isArray (t){
            return _classOf(t).toLowerCase() == 'array';
        }
        var copyIsArray,
            self = this;
        ...
        _extend = function(target, options, deep) {
            for (name in options) {
                src = target[name];
                copy = options[name]; 
                 if (target === copy) { continue; } 
                 if (deep && copy
                        && (isPlainObject(copy) || (copyIsArray = _isArray(copy)))) {
                    if (copyIsArray) {
                        copyIsArray = false;
    ...
    objToArray: function(obj, filter){
        var list = [];
        this.each(obj, function(k, v){
            filter ? filter(v) && list.push(v) : list.push(v)
        });
        return list
    },
    ...
    directToObj: function(str){
        //@view:clinic/index@url:/site/patient
        var list  = str.split('@'),
            _obj = {};
        this.each(list, function(i, v){
            if(v != ''){
                var d = v.split(/:/);
                _obj[d[0]] = d[1];
            }
